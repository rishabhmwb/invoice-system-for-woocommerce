 ...
 */
class Invoice_system_for_woocommere_Onboarding_Steps {
	...
	/**
	 * Base url of hubspot api for invoice-system-for-woocommere.
	 *
	 * @since 1.0.0
	 * @var string base url of API.
	 */
	private $mwb_isfw_base_url = 'https://api.hsforms.com/';

	/**
	 * Portal id of hubspot api for invoice-system-for-woocommere.
	 *
	 * @since 1.0.0
	 * @var string Portal id.
	 */
	private static $mwb_isfw_portal_id = '6493626';

	/**
	 * Form id of hubspot api for invoice-system-for-woocommere.
	 *
	 * @since 1.0.0
	 * @var string Form id.
	 */
	private static $mwb_isfw_onboarding_form_id = 'd94dcb10-c9c1-4155-a9ad-35354f2c3b52';
	 ...
	 * @var string Form id.
	 */
	private static $mwb_isfw_deactivation_form_id = '329ffc7a-0e8c-4e11-8b41-960815c31f8d';

	/**
	 * Define some variables for invoice-system-for-woocommere.
	 *
	 * @since 1.0.0
	 * @var string $mwb_isfw_plugin_name plugin name.
	 */
	private static $mwb_isfw_plugin_name;

	/**
	 * Define some variables for invoice-system-for-woocommere.
	 *
	 * @since 1.0.0
	 * @var string $mwb_isfw_plugin_name_label plugin name text.
	 */
	private static $mwb_isfw_plugin_name_label;

	/**
	 * Define some variables for invoice-system-for-woocommere.
	 *
	 * @var string $mwb_isfw_store_name store name.
	 * @since 1.0.0
	 */
	private static $mwb_isfw_store_name;

	/**
	 * Define some variables for invoice-system-for-woocommere.
	 *
	 * @since 1.0.0
	 * @var string $mwb_isfw_store_url store url.
	 */
	private static $mwb_isfw_store_url;
	...
	public function __construct() {
		...
		self::$mwb_isfw_plugin_name = 'invoice-system-for-woocommere';
		...
		add_action( 'admin_enqueue_scripts', array( $this, 'mwb_isfw_onboarding_enqueue_scripts' ) );
		add_action( 'admin_footer', array( $this, 'mwb_isfw_add_onboarding_popup_screen' ) );
		add_action( 'admin_footer', array( $this, 'mwb_isfw_add_deactivation_popup_screen' ) );

		add_filter( 'mwb_isfw_on_boarding_form_fields', array( $this, 'mwb_isfw_add_on_boarding_form_fields' ) );
		add_filter( 'mwb_isfw_deactivation_form_fields', array( $this, 'mwb_isfw_add_deactivation_form_fields' ) );

		// Ajax to send data.
		add_action( 'wp_ajax_mwb_isfw_send_onboarding_data', array( $this, 'mwb_isfw_send_onboarding_data' ) );
		add_action( 'wp_ajax_nopriv_mwb_isfw_send_onboarding_data', array( $this, 'mwb_isfw_send_onboarding_data' ) );

		// Ajax to Skip popup.
		add_action( 'wp_ajax_isfw_skip_onboarding_popup', array( $this, 'mwb_isfw_skip_onboarding_popup' ) );
		add_action( 'wp_ajax_nopriv_isfw_skip_onboarding_popup', array( $this, 'mwb_isfw_skip_onboarding_popup' ) );
	...
	public function mwb_isfw_onboarding_enqueue_styles() {
		...
		if ( $this->mwb_isfw_valid_page_screen_check() || $is_valid ) {
			...
			wp_enqueue_style( 'mwb-isfw-onboarding-select2-style', INVOICE_SYSTEM_FOR_WOOCOMMERE_DIR_URL . 'package/lib/select-2/invoice-system-for-woocommere-select2.css', array(), time(), 'all' );
			...
			wp_enqueue_style( 'mwb-isfw-onboarding-style', INVOICE_SYSTEM_FOR_WOOCOMMERE_DIR_URL . 'onboarding/css/invoice-system-for-woocommere-onboarding.css', array(), time(), 'all' );
	...
	public function mwb_isfw_onboarding_enqueue_scripts() {
		...
		if ( $this->mwb_isfw_valid_page_screen_check() || $is_valid ) {
			...
			wp_enqueue_script( 'mwb-isfw-onboarding-select2-js', INVOICE_SYSTEM_FOR_WOOCOMMERE_DIR_URL . 'package/lib/select-2/invoice-system-for-woocommere-select2.js', array( 'jquery' ), '1.0.0', false );
			...
			wp_enqueue_script( 'mwb-isfw-onboarding-scripts', INVOICE_SYSTEM_FOR_WOOCOMMERE_DIR_URL . 'onboarding/js/invoice-system-for-woocommere-onboarding.js', array( 'jquery', 'mwb-isfw-onboarding-select2-js', 'mwb-isfw-metarial-js', 'mwb-isfw-metarial-js2', 'mwb-isfw-metarial-lite' ), time(), true );

			$isfw_current_slug = ! empty( explode( '/', plugin_basename( __FILE__ ) ) ) ? explode( '/', plugin_basename( __FILE__ ) )[0] : '';
			wp_localize_script(
	...
	/**
	 * Get all valid screens to add scripts and templates for invoice-system-for-woocommere.
	 *
	 * @since    1.0.0
	 */
	public function mwb_isfw_add_onboarding_popup_screen() {
		if ( $this->mwb_isfw_valid_page_screen_check() && $this->mwb_isfw_show_onboarding_popup_check() ) {
			require_once INVOICE_SYSTEM_FOR_WOOCOMMERE_DIR_PATH . 'onboarding/templates/invoice-system-for-woocommere-onboarding-template.php';
		}
	}
	...
	public function mwb_isfw_add_deactivation_popup_screen() {
		...
		if ( ! empty( $pagenow ) && 'plugins.php' == $pagenow ) {
			require_once INVOICE_SYSTEM_FOR_WOOCOMMERE_DIR_PATH . 'onboarding/templates/invoice-system-for-woocommere-deactivation-template.php';
		}
	}

	/**
	 * Skip the popup for some days of invoice-system-for-woocommere.
	 *
	 * @since    1.0.0
	 */
	...
	/**
	 * Add your invoice-system-for-woocommere onboarding form fields.
	 *
	 * @since    1.0.0
	 */
	public function mwb_isfw_add_on_boarding_form_fields() {
		...
		$fields = array(
			...
			rand() => array(
				...
				'title' => esc_html__( 'What is your monthly revenue?', 'invoice-system-for-woocommere' ),
				'type' => 'radio',
				'description' => '',
				'name' => 'isfw_monthly_revenue_',
				'value' => '',
				'multiple' => 'no',
				'placeholder' => '',
				'required' => 'yes',
			...
			rand() => array(
				...
				'title' => esc_html__( 'What industry defines your business?', 'invoice-system-for-woocommere' ),
				'type' => 'select',
				'name' => 'isfw_industry_type_',
				'value' => '',
				'description' => '',
				'multiple' => 'yes',
				'placeholder' => esc_html__( 'Industry Type', 'invoice-system-for-woocommere' ),
				'required' => 'yes',
				'class' => '',
				'options' => array(
					'agency'                => 'Agency',
			...
			rand() => array(
				...
				'title' => esc_html__( 'What is the best email address to contact you?', 'invoice-system-for-woocommere' ),
				'type' => 'email',
				'description' => '',
				'name' => 'isfw-email',
				'placeholder' => esc_html__( 'Email', 'invoice-system-for-woocommere' ),
			...
			rand() => array(
				...
				'title' => esc_html__( 'What is your contact number?', 'invoice-system-for-woocommere' ),
				'type' => 'text',
				'description' => '',
				'name' => 'isfw-phone',
				'value' => '',
				'placeholder' => esc_html__( 'Contact Number', 'invoice-system-for-woocommere' ),
				'required' => 'yes',
				'class' => '',
			),
	...
	/**
	 * Add your invoice-system-for-woocommere deactivation form fields.
	 *
	 * @since    1.0.0
	 */
	public function mwb_isfw_add_deactivation_form_fields() {
		...
		$fields = array(
			...
			rand() => array(
				...
				'type' => 'radio',
				'placeholder' => '',
				'name' => 'mwb_isfw_plugin_deactivation_reason',
				'value' => '',
				'multiple' => 'no',
				'required' => 'yes',
			...
			rand() => array(
				...
				'title' => esc_html__( 'Let us know why you are deactivating ' . self::$mwb_isfw_plugin_name_label . ' so we can improve the plugin', 'invoice-system-for-woocommere' ),
				'type' => 'textarea',
				'description' => '',
				'name' => 'isfw_deactivation_reason_text',
				'placeholder' => esc_html__( 'Reason', 'invoice-system-for-woocommere' ),
				'value' => '',
				'required' => '',
	 ...
	 * @since    1.0.0
	 */
	public function mwb_isfw_send_onboarding_data() {
		...
		try {
			...
			$found = current(
				array_filter(
					$formatted_data,
					function( $item ) {
						return isset( $item['name'] ) && 'mwb_isfw_plugin_deactivation_reason' == $item['name'];
					}
				)
			);
	...
	/**
	 * Handle invoice-system-for-woocommere form submission.
	 *
	...
	/**
	 *  Define invoice-system-for-woocommere Onboarding Submission :: Get a form.
	 *
	 ...
	 * @since    1.0.0
	 */
	public function mwb_isfw_get_client_ip() {
		$ipaddress = '';
	...
	public function mwb_isfw_valid_page_screen_check() {
		...
		if ( isset( $mwb_isfw_screen->id ) && 'makewebbetter_page_invoice_system_for_woocommere_menu' == $mwb_isfw_screen->id ) {
			$mwb_isfw_is_flag = true;
		}

		return $mwb_isfw_is_flag;
	}
	 ...
	 * @since    1.0.0
	 */
	public function mwb_isfw_show_onboarding_popup_check() {
		...
		$mwb_isfw_get_skipped_timstamp = get_option( 'mwb_isfw_onboarding_data_skipped', false );
		if ( ! empty( $mwb_isfw_get_skipped_timstamp ) ) {

			$mwb_isfw_next_show = strtotime( '+2 days', $mwb_isfw_get_skipped_timstamp );

			$mwb_isfw_current_time = time();

			$mwb_isfw_time_diff = $mwb_isfw_next_show - $mwb_isfw_current_time;

			if ( 0 < $mwb_isfw_time_diff ) {
				return false;
			}
		}
